...
def run_CFVAE_C():
    ...
    parser.add_argument('--sensitive_latent_size', default=3, type=int)
    ...
    if config.dataset == 'data':
        train_data, train_label, test_data, test_data_cf, test_label, test_label_cf = load_data()
    ...
    for train_n in range(config.train_num):
        ...
        for epoch_number in range(config.train_epochs):
            ...
            for (data, label) in train_loader:
                ...
                if config.model_name == 'VAE': # Original version of VAE
                    ...
                    q_za_mean, q_za_log_sigma = model.encoder_a(data[:, 0:4])
                    za = reparam(q_za_mean, q_za_log_sigma)
                    reconst_za = model.decoderA(za)

                    reconst_loss_x = torch.nn.functional.mse_loss(reconst_zx,data[:,4:])
                    reconst_loss_a = torch.nn.functional.mse_loss(reconst_za,data[:,0:4])
        ...
        torch.save(model, './model/CFAVE_C')

        plt.plot(loss_epoch_z)
        plt.xlabel('epoch')
        plt.ylabel('loss')
        plt.savefig('./pic/CFAVE_C.png')
